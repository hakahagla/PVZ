...
#Import assests
#bg is main menu background
#bg2 is game menu background
...
#Scaling pictures to fit the screen
...
class game():
    ...
    def __init__(self):
        ...
        c_coord = [[300,150],[380,150],[460,150],[540,150],[620,150],[700,150],[780,150],[870,150],[960,150],
    ...
    def place(self):
        ...
        if s_count.counter - 100 >= 0: #Replace 100 with an variable which the function takes in first and stores it to that
            ...
            for i in range (0,len(c_coord)):
                self.diffx = c_coord[i][0]-self.x
                self.diffy = c_coord[i][1]-self.y
                self.temp = (self.diffx**2+self.diffy**2)**1/2
                if self.temp <= self.distance:
                    self.targetx,self.targety = c_coord[i][0],c_coord[i][1]
...
class wave():
    ...
    def __init__(self):
        ...
        self.time = 0
        self.framevalue = 0
        self.x,self.y = 860,615 #Storing location of flag meter
        self.spawntime = [5,15,15,15,15,15,20,15,15,15,15,10,10]
        self.wavecount = 0
        self.changed = False
        self.zombie_loc = [[1150,150],[1150,260],[1150,370],[1150,480],[1150,590]]


    def count_time(self):
        self.temp += self.clock.tick_busy_loop(240)
        
        if self.temp // 1000 == 1: #Set count in seconds so the game dont need to count in miliseconds which will decrease performance
            self.time += 1 
            self.temp = 0
        if len(self.spawntime) != 0:
            if self.time == self.spawntime[0]:
                self.changed = False
                self.animation()
                self.spawntime.remove(self.time)
                self.time = 0
                self.wavecount += 1
                
                if len(self.spawntime) == 0 or self.wavecount == 7:
                    self.big()
                else:
                    self.normal()
        self.animation()
    ...
    def normal(self):
        self.amount = random.randint(2,3)
        self.spawn()

    def big(self):
        self.amount = random.randint(5,7)
        self.spawn()
        print(True)
    ...
    def spawn(self):
        ...
        for i in range(0,self.amount):
            ...
            for j in range(1,999):
                if j not in created_zombie:
                    created_zombie.append(j)
                    globals()['objz'+str(j)] = normalzombie(self.zombie_loc[self.row][0]-30,self.zombie_loc[self.row][1]-50,j)
                    break 

    def animation(self):
        global flagmeter_anim
        if self.framevalue < len(flagmeter_anim):
            self.currframe = flagmeter_anim[self.framevalue]
            if self.time == self.spawntime[0] and self.changed == False:
                self.framevalue += 1
                self.changed = True
            menu.screen.blit(self.currframe,(self.x,self.y))
...
flagmeter_anim=[pygame.image.load("Screen/flagmeter1.png").convert_alpha(),
pygame.image.load("Screen/flagmeter2.png").convert_alpha(),
pygame.image.load("Screen/flagmeter3.png").convert_alpha(),
pygame.image.load("Screen/flagmeter4.png").convert_alpha(),
pygame.image.load("Screen/flagmeter5.png").convert_alpha(),
pygame.image.load("Screen/flagmeter6.png").convert_alpha(),
pygame.image.load("Screen/flagmeter7.png").convert_alpha(), #Big wave 1
pygame.image.load("Screen/flagmeter8.png").convert_alpha(),
pygame.image.load("Screen/flagmeter9.png").convert_alpha(),
pygame.image.load("Screen/flagmeter10.png").convert_alpha(),
pygame.image.load("Screen/flagmeter11.png").convert_alpha(),
pygame.image.load("Screen/flagmeter12.png").convert_alpha(),
pygame.image.load("Screen/flagmeter13.png").convert_alpha()] #Big wave 2
...
c_coord = [[300,150],[380,150],[460,150],[540,150],[620,150],[700,150],[780,150],[870,150],[960,150], #Stores the centre of all tiles
                                [300,260],[380,260],[460,260],[540,260],[620,260],[700,260],[780,260],[870,260],[960,260],
                                [300,370],[380,370],[460,370],[540,370],[620,370],[700,370],[780,370],[870,370],[960,370],
                                [300,480],[380,480],[460,480],[540,480],[620,480],[700,480],[780,480],[870,480],[960,480],
                                [300,590],[380,590],[460,590],[540,590],[620,590],[700,590],[780,590],[870,590],[960,590]]
